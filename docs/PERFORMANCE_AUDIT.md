...
The initial JavaScript is 1,852,793 bytes (317,282 bytes gzip), down 36.7% from the audited v0.9 baseline of 2,925,687 bytes. Paper Lab and PDF.js remain lazy feature payloads rather than startup work.

| Status | Metric | Observed | Budget |
|---|---|---:|---:|
| PASS | Initial JavaScript | 1,852,793 bytes | < 1,900,000 bytes |
| PASS | Initial CSS | 52,516 bytes | < 70,000 bytes |
| PASS | Training content chunk | 81,995 bytes | < 150,000 bytes |
| PASS | Scheduler mean | 0.1307 ms | < 2 ms |

## 🧩 Lazy boundaries

| Chunk | Bytes | Why isolated |
|---|---:|---|
| Paper Lab | 862,770 | PDF renderer loads only when the user opens Paper Lab. |
...
| CSS | 52,516 | One theme-aware stylesheet with responsive rules. |

## ⚙️ Runtime checks

- Today scheduling averaged 0.1307 ms over 2,000 deterministic runs.
