...
The initial JavaScript is 1,038,344 bytes (217,890 bytes gzip), down 64.5% from the audited v0.9 baseline of 2,925,687 bytes. Paper Lab and PDF.js remain lazy feature payloads rather than startup work.

| Status | Metric | Observed | Budget |
|---|---|---:|---:|
| PASS | Initial JavaScript | 1,038,344 bytes | < 1,900,000 bytes |
| PASS | Initial CSS | 83,074 bytes | < 85,000 bytes |
| PASS | Content inventory chunk | 81,995 bytes | < 150,000 bytes |
| PASS | Scheduler mean | 0.0040 ms | < 2 ms |

## 🧩 Lazy boundaries

| Chunk | Bytes | Why isolated |
|---|---:|---|
| Paper Lab | 866,491 | PDF renderer loads only when the user opens Paper Lab. |
...
| CSS | 83,074 | One theme-aware stylesheet with responsive rules. |

## ⚙️ Runtime checks

- Today scheduling averaged 0.0040 ms over 2,000 deterministic runs.
