...
The initial JavaScript is 830,599 bytes (162,243 bytes gzip), down 71.6% from the audited v0.9 baseline of 2,925,687 bytes. Paper Lab and PDF.js remain lazy feature payloads rather than startup work.

| Status | Metric | Observed | Budget |
|---|---|---:|---:|
| PASS | Initial JavaScript | 830,599 bytes | < 1,900,000 bytes |
| PASS | Initial CSS | 62,691 bytes | < 70,000 bytes |
| PASS | Training content chunk | 81,995 bytes | < 150,000 bytes |
| PASS | Scheduler mean | 0.0707 ms | < 2 ms |

## 🧩 Lazy boundaries

| Chunk | Bytes | Why isolated |
|---|---:|---|
| Paper Lab | 860,751 | PDF renderer loads only when the user opens Paper Lab. |
...
| CSS | 62,691 | One theme-aware stylesheet with responsive rules. |

## ⚙️ Runtime checks

- Today scheduling averaged 0.0707 ms over 2,000 deterministic runs.
