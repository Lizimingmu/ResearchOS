...
The initial JavaScript is 663,763 bytes (121,760 bytes gzip), down 77.3% from the audited v0.9 baseline of 2,925,687 bytes. Paper Lab and PDF.js remain lazy feature payloads rather than startup work.

| Status | Metric | Observed | Budget |
|---|---|---:|---:|
| PASS | Initial JavaScript | 663,763 bytes | < 1,900,000 bytes |
| PASS | Initial CSS | 52,919 bytes | < 70,000 bytes |
| PASS | Training content chunk | 81,995 bytes | < 150,000 bytes |
| PASS | Scheduler mean | 0.1240 ms | < 2 ms |

## 🧩 Lazy boundaries

| Chunk | Bytes | Why isolated |
|---|---:|---|
| Paper Lab | 859,540 | PDF renderer loads only when the user opens Paper Lab. |
