...
- Blockers: localization audit failure; uncommitted scientific paraphrases need changeset completion and Codex scientific review. M012 OpenCode invocation remains blocked by the Codex host's nested-process policy: even with project snapshots disabled and Git denied, OpenCode initialization returns `EPERM: uv_spawn 'git'`. Run the handoff command from a normal user terminal or a host that permits OpenCode child processes.
- Current milestone: M013 Research Problem Atlas and Source-first infrastructure. M012 Protocol Lab specification is preserved but implementation/content generation is paused until M013 establishes source registry, claim mapping and ingestion gates. M011 localization remains an unapproved in-progress dependency.
