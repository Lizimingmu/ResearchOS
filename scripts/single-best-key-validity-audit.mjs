...
for(const asset of assets){
  ...
  if(!singleContracts.has(asset.taskContract)&&asset.expectedOptionIds.length!==1)continue;
  ...
  if(asset.options.filter(option=>option.id===expectedId).length!==1)errors.push(`${asset.id}: expected option is missing or duplicated`);
  const evidence=asset.scoringRule.evidenceExpectations.find(entry=>entry.optionId===expectedId);
  if(!evidence?.allowedRowIds.length||evidence.allowedRowIds.length!==evidence.requiredFactFragmentsCn.length)errors.push(`${asset.id}: missing concrete evidence basis`);
  else for(const [index,rowId] of evidence.allowedRowIds.entries()) {
    const row=asset.stimulus.rowsCn.find(row=>row[0]===rowId);
    const normalized=(row?.[1]??'').replace(/[\s，。；：、“”‘’（）()\-—]/g,'');
    if(!row||!evidence.requiredFactFragmentsCn[index]||!normalized.includes(evidence.requiredFactFragmentsCn[index]))errors.push(`${asset.id}: evidence fragment does not resolve to ${rowId}`);
  }
...
for(const contract of singleContracts)if((byContract.get(contract)?.size??0)<2)errors.push(`${contract}: answer key still collapses to one semantic slot or is absent`);
