...
export function serializeLearningData(state: AppStateData, format: LearningExportFormat): { content: string; mime: string; extension: string } {
  ...
  if (format === "json") return { content: `${JSON.stringify({ exportedAt, schemaVersion: state.schemaVersion, learningEvents: state.learningEvents, knowledgeWorkspace: state.knowledgeWorkspace, responses: state.responses, reviewLogs: state.reviewLogs, skillEvidence: state.skillEvidence, misconceptions: state.misconceptions, assessments: state.assessmentHistory }, null, 2)}\n`, mime: "application/json", extension: "json" };
  if (format === "csv") {
