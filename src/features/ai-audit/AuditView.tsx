...
import { auditDisplay, bilingualAuditTitle } from "../../i18n/scientificContent";
...
export function AuditView() {
  ...
  const display = auditDisplay(current);
  ...
  return <div className="split-page audit-page"><aside className="feature-list-pane"><div className="pane-heading"><span className="eyebrow">AI 审核案例</span><strong>{auditCases.length}</strong></div><label className="compact-search"><Search size={13} /><span>使用工作区搜索筛选</span></label><div className="audit-case-list">{filtered.map((entry) => <button key={entry.id} className={entry.id === current.id ? "active" : ""} onClick={() => { selectAudit(entry.id); setDecisions({}); setReasons({}); setTransfer(""); }}><span>{bilingualAuditTitle(entry)}<small>{entry.domain}</small></span><i>{entry.steps.length} 个步骤</i></button>)}</div></aside><section className="feature-workspace scrollable"><header className="detail-header"><div><span className="eyebrow">AI 科研监督 · {current.domain.toUpperCase()}</span><h1>{display.title}</h1><p>{display.task}</p></div><AlertTriangle size={24} /></header><div className="case-context"><span>研究情境</span>{display.context}<details><summary>查看英文原始案例</summary><p>{current.context}</p></details></div><div className="audit-plan"><div className="audit-plan-head"><span>AI 提议方案</span><span>你的处置</span><span>理由</span></div>{display.steps.map((step, index) => { const savedDecision = existing ? step.expected : decisions[step.id]; return <div key={step.id} className="audit-step"><div><i>{index + 1}</i><p>{step.text}</p></div><div className="decision-buttons">{(["approve", "question", "reject"] as Decision[]).map((decision) => { const Icon = decision === "approve" ? CheckCircle2 : decision === "question" ? CircleHelp : XCircle; return <button key={decision} disabled={locked} className={savedDecision === decision ? `active ${decision}` : ""} onClick={() => { const next = { ...decisions, [step.id]: decision }; setDecisions(next); persistAuditDraft(next); }} title={{ approve: "合理", question: "需要核查", reject: "存在问题" }[decision]}><Icon size={15} /></button>; })}</div><textarea disabled={locked} rows={2} value={locked ? (step.expected === "approve" ? "该步骤原则上合理。" : step.expected === "question" ? "该步骤需要进一步核查适用前提。" : "该步骤存在方法学问题。") : (reasons[step.id] ?? "")} onChange={(event) => { const next = { ...reasons, [step.id]: event.target.value }; setReasons(next); persistAuditDraft(decisions, next); }} placeholder="方法学理由…" /></div>; })}</div>{!locked && <div className="audit-submit"><label>总体信心<select value={confidence} onChange={(event) => { const next = Number(event.target.value) as Confidence; setConfidence(next); persistAuditDraft(decisions, reasons, next); }}>{[1,2,3,4].map((value) => <option key={value} value={value}>{value}</option>)}</select></label><button className="primary" onClick={submit}>锁定审查</button></div>}{locked && <><section className="feedback-stage senior-audit"><div className="stage-kicker"><span>审稿人级评估</span></div><div className="senior-review"><div><span>识别与处理建议</span><p>{display.summary}</p></div><div><span>遗漏问题</span><ul>{display.missedRisks.map((risk) => <li key={risk}>{risk}</li>)}</ul></div>{current.steps.filter((step) => step.issue).map((step) => <div key={step.id}><span>{{ approve: "合理", question: "需要核查", reject: "存在问题" }[step.expected]} · {{ minor: "轻微问题", major: "重大问题", critical: "致命 / 结构性问题" }[step.severity ?? "major"]}</span><p>{step.expected === "reject" ? "该步骤可能破坏研究设计、统计推断或证据边界，应在执行前重构。" : "请补充适用条件、诊断与不确定性说明。"}</p></div>)}</div></section><section className="evidence-stage"><div className="stage-kicker"><span>证据</span></div><EvidenceList sourceIds={current.sourceIds} /></section><section className="transfer-stage"><div className="stage-kicker"><span>迁移</span></div><h3>{display.transfer}</h3><textarea rows={3} value={transfer} onChange={(event) => { setTransfer(event.target.value); persistAuditDraft(decisions, reasons, confidence, event.target.value); }} /><div className="attempt-controls"><label>项目<select value={projectId} onChange={(event) => { setProjectId(event.target.value); persistAuditDraft(decisions, reasons, confidence, transfer, event.target.value); }}><option value="">不关联项目</option>{projects.map((project) => <option key={project.id} value={project.id}>{project.name}</option>)}</select></label><button onClick={recordTransfer}>保存迁移</button></div></section></>}</section></div>;
}
