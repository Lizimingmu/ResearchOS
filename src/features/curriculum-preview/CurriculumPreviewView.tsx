...
function PreviewDetail({ tab, item }: { tab: Tab; item: ReturnType<typeof listFor>[number] }) {
  ...
  if (tab === "method") {
    ...
    return <article><span className="eyebrow">Method Lesson · {lesson.estimatedMinutes} min · ten-dimension contract</span><h2>{lesson.titleCn}</h2><p>{lesson.scientificQuestionCn}</p><section><h3>直觉与 worked example</h3><p>{lesson.intuitionCn}</p><blockquote>{lesson.workedExampleCn}</blockquote><ol>{lesson.walkthroughStepsCn.map((entry) => <li key={entry}>{entry}</li>)}</ol></section><div className="method-grid"><section><h3>Inputs</h3><ul>{lesson.inputsCn.map((entry) => <li key={entry}>{entry}</li>)}</ul></section><section><h3>Outputs</h3><ul>{lesson.outputsCn.map((entry) => <li key={entry}>{entry}</li>)}</ul></section><section><h3>Assumptions</h3><ul>{lesson.assumptionsCn.map((entry) => <li key={entry}>{entry}</li>)}</ul></section><section><h3>Reviewer checks</h3><ul>{lesson.reviewerChecksCn.map((entry) => <li key={entry}>{entry}</li>)}</ul></section></div><section><h3>论文阅读练习</h3><blockquote>{lesson.paperReadingExample.snippetCn}</blockquote><ul>{lesson.paperReadingExample.readerChecksCn.map((entry) => <li key={entry}>{entry}</li>)}</ul></section><section><h3>方法选择对照</h3>{lesson.methodComparisonCn.map((comparison) => <div key={comparison.alternativeCn}><strong>对照：{comparison.alternativeCn}</strong><p>选择本方法：{comparison.chooseThisWhenCn}</p><p>改用对照方法：{comparison.chooseAlternativeWhenCn}</p></div>)}</section><p>{lesson.paperAppearanceCn}</p>{[lesson.primaryApply, lesson.remediation, lesson.delayedReview].map((asset) => <AssetPreview key={asset.id} asset={asset}/>)}</article>;
  }
  if (tab === "case") {
    const caseLab = item as (typeof stagedCaseLabs)[number];
    return <article><span className="eyebrow">Case Lab · {caseLab.theme}</span><h2>{caseLab.titleCn}</h2><p>{caseLab.initialContextCn}</p>{caseLab.stages.map((stage) => <section className="preview-case-stage" key={stage.id}><h3>{stage.titleCn}</h3><p>{stage.evidenceCn}</p><strong>{stage.reasoningPromptCn}</strong><div className="preview-information-update" aria-label="信息状态变化"><p><b>增强：</b>{stage.informationUpdate.strengthenedCn.join("；")}</p><p><b>削弱：</b>{stage.informationUpdate.weakenedCn.join("；")}</p><p><b>仍未解决：</b>{stage.informationUpdate.unresolvedCn.join("；")}</p><p><b>迫使更新的证据：</b>{stage.informationUpdate.forcingEvidenceCn}</p></div><small>Calibration: {stage.calibrationCn}</small><small>Update: {stage.updatePromptCn}</small></section>)}<h3>Final task</h3><ul>{caseLab.finalTaskCn.map((entry) => <li key={entry}>{entry}</li>)}</ul></article>;
  }
