...
function PreviewDetail({ tab, item }: { tab: Tab; item: ReturnType<typeof listFor>[number] }) {
  ...
  if (tab === "method") {
    ...
    return <article><span className="eyebrow">Method Lesson · ten-dimension contract</span><h2>{lesson.titleCn}</h2><p>{lesson.scientificQuestionCn}</p><div className="method-grid"><section><h3>Inputs</h3><ul>{lesson.inputsCn.map((entry) => <li key={entry}>{entry}</li>)}</ul></section><section><h3>Outputs</h3><ul>{lesson.outputsCn.map((entry) => <li key={entry}>{entry}</li>)}</ul></section><section><h3>Assumptions</h3><ul>{lesson.assumptionsCn.map((entry) => <li key={entry}>{entry}</li>)}</ul></section><section><h3>Reviewer checks</h3><ul>{lesson.reviewerChecksCn.map((entry) => <li key={entry}>{entry}</li>)}</ul></section></div><p>{lesson.paperAppearanceCn}</p>{[lesson.primaryApply, lesson.remediation, lesson.delayedReview].map((asset) => <AssetPreview key={asset.id} asset={asset}/>)}</article>;
  }
  if (tab === "case") {
