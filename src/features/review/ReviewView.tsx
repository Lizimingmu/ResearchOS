...
function DueReview({ item }: { item: ReviewItem }) {
  ...
  const rate = (correctness: number) => {
    ...
    addSkillEvidence({ skillId: item.conceptType === "audit" ? "ai-oversight" : item.conceptType === "paper" ? "patterns" : "methods", taskId: item.conceptId, score: correctness, delayed: true, blindTransfer: false, confidence });
...
export function ReviewView() {
  ...
  const currentReview = reviewItems.find((item) => item.id === selectedReview) ?? due[0];
