...
Final: ${JSON.stringify(session.finalResponse)}`, claimBoundary: session.finalResponse?.["最大可辩护结论"] ?? timeline.at(-1)?.claimBoundary, nextStep: session.finalResponse?.["下一项最小充分分析或验证"], linkedLearningEventIds: [] });
  ...
  if (!session && missing.length) return <div className="page case-lab"><header className="page-header"><div><span className="eyebrow">Case Lab · prerequisite gate</span><h1>{researchCase.titleCn}</h1><p>此案例要求先完成标准化先修能力；项目相关性、Guide 深链或直接导航都不能绕过。</p></div></header><section className="learning-wait"><h2>缺少先修能力</h2><ul>{missing.map((item) => <li key={item.id}><strong>{item.titleCn}</strong><button onClick={() => openLearningContentTask({ contentId: item.id, activityType: "explanation" })}>前往学习</button></li>)}</ul></section></div>;
