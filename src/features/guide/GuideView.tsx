...
export function GuideView() {
  ...
  return <div className="guide-layout">
    ...
    <article className="guide-reader scrollable" id={section.anchor}><header><span className="eyebrow">{chapterNames[section.chapterId]} · {section.classification}</span><h1>{section.titleCn}</h1><p>{section.titleEn}</p><div className="provenance-strip">{section.verificationStatus} · revision {section.revision} · sources {section.evidenceSourceIds.join(", ")}</div>{section.verificationStatus === "pending" && <div className="pending-curriculum-banner">待科学审核 · 不进入正式 Today · 不计标准化能力</div>}</header><blockquote>{section.summaryCn}</blockquote>{section.bodyCn.map((paragraph) => <p key={paragraph}>{paragraph}</p>)}{section.glossaryTerms.length > 0 && <section className="guide-glossary"><h2>Glossary</h2>{section.glossaryTerms.map((term) => <dl key={term.en}><dt>{term.zh} <span>{term.en}</span></dt><dd>{term.definitionCn}</dd></dl>)}</section>}{section.advancedNotes && <details><summary>进阶说明</summary>{section.advancedNotes.map((note) => <p key={note}>{note}</p>)}</details>}<div className="guide-links">{section.links.map((link) => <button key={link.targetId} onClick={() => ["case_lab", "concept_lesson", "method_lesson"].includes(link.type) ? openLearningContentTask({ contentId: link.targetId, activityType: link.type === "case_lab" ? "independent_case" : "explanation" }) : setView("learning")}><Link2 size={14}/>{link.labelCn}</button>)}</div><button className="primary" disabled={readIds.includes(section.id)} onClick={() => markRead(section.id)}><BookOpen size={14}/>{readIds.includes(section.id) ? "已标记阅读（不计能力）" : "标记为已读"}</button></article>
  </div>;
}
