...
export function SettingsView() {
  ...
  const exportDb = async () => {
    try { const path = await saveDialog({ defaultPath: `ResearchOS-v0.11.0-backup-${new Date().toISOString().slice(0,10)}.sqlite3`, filters: [{ name: "SQLite 数据库", extensions: ["sqlite3"] }] }); if (path) { await exportBackup(path); notify(`备份已导出到 ${path}`, "success"); } }
