...
export function SettingsView() {
  ...
  const routineSettings = useAppStore((state) => state.routineSettings);
  const updateRoutineSettings = useAppStore((state) => state.updateRoutineSettings);
  ...
  return (
    <div className="settings-layout">
      ...
      <section className="settings-panel scrollable">
        ...
        {tab === "learning" && <div className="settings-group"><h2>科研 Routine</h2><p className="setting-footnote">这些是可修改的轻量默认值，不是硬性打卡。</p><div className="form-grid two"><label>每周论文<input type="number" min={0} max={14} value={routineSettings.weeklyPapers} onChange={(event)=>updateRoutineSettings({weeklyPapers:Number(event.target.value)})}/></label><label>每周概念<input type="number" min={0} max={14} value={routineSettings.weeklyConcepts} onChange={(event)=>updateRoutineSettings({weeklyConcepts:Number(event.target.value)})}/></label><label>每周研究日志<input type="number" min={0} max={14} value={routineSettings.weeklyProjectReflections} onChange={(event)=>updateRoutineSettings({weeklyProjectReflections:Number(event.target.value)})}/></label><label>每月回顾<input type="number" min={0} max={8} value={routineSettings.monthlyCompetenceReviews} onChange={(event)=>updateRoutineSettings({monthlyCompetenceReviews:Number(event.target.value)})}/></label></div><h2>调度权重</h2>{Object.entries(settings.weights).map(([keyName, value]) => <label key={keyName}>{weightLabel(keyName)}<div className="range-input"><input type="range" min={0} max={1} step={.05} value={value} onChange={(event) => updateWeights({ [keyName]: Number(event.target.value) })} /><output>{value.toFixed(2)}</output></div></label>)}<div className="setting-note"><ShieldCheck size={15} /><p>错误且高信心的作答会形成明确的错误观念，并安排次日陌生变式提取。只有正确完成变式才能解除。</p></div></div>}
