...
export function SettingsView() {
  ...
  const exportDb = async () => {
    try { const path = await saveDialog({ defaultPath: `ResearchOS-v0.10.1-backup-${new Date().toISOString().slice(0,10)}.sqlite3`, filters: [{ name: "SQLite database", extensions: ["sqlite3"] }] }); if (path) { await exportBackup(path); notify(`Backup exported to ${path}`, "success"); } }
