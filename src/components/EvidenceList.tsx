      ...
      {sourceIds.map((id) => evidenceById[id]).filter(Boolean).map((source) => (
        <article key={source.id} className="evidence-row">
          ...
          <div>
            ...
            <span>Tier {source.tier} · {source.sourceName} · {source.verificationStatus}</span>
            ...
            <div className="evidence-identifiers">
              ...
              {source.url && <a href={source.url} target="_blank" rel="noreferrer">Source <ExternalLink size={11} /></a>}
            </div>
          </div>
        </article>
      ))}
